...
class StackBlockPyramidSeqUnseenColors(Task):
    ...
    def __init__(self, lang_goals):
        super().__init__()
        self.max_steps = 12
#         self.lang_template = "put the {pick} block on {place}"
        self.lang_template = lang_goals
...
#         random.shuffle(color_names)
        ...
        # Load language goal
        lang_goals = self.lang_template
        for i in range(len(lang_goals)):
            lang_goal = lang_goals[i]
            words = lang_goal.split(" ")
            obj_idx = color_names.index(words[2])
            obj = objs[obj_idx]
            targ = targs[i]
            self.goals.append(([obj], np.ones((1, 1)), [targ],
